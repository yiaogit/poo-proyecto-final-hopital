...
def print_cita(cita_idx):
    ...
    if request.args.get("format") == "pdf":
        return _generar_pdf("print_cita.html", ctx, f"cita_{cita.paciente.dni}_{cita_idx}.pdf")
...
def print_factura(cita_idx):
    ...
    if request.args.get("format") == "pdf":
        return _generar_pdf("print_factura.html", ctx, f"factura_{cita.paciente.dni}_{cita_idx}.pdf")
...
def print_informe(dni):
    ...
    if request.args.get("format") == "pdf":
        return _generar_pdf("print_informe.html", ctx, f"informe_{paciente.dni}.pdf")
...
def print_agenda(id_colegiado):
    ...
    if request.args.get("format") == "pdf":
        return _generar_pdf("print_agenda_medico.html", ctx, f"agenda_{medico.identificacion}.pdf")
...
def print_nomina(id_colegiado):
    ...
    if request.args.get("format") == "pdf":
        return _generar_pdf("print_nomina.html", ctx, f"nomina_{medico.identificacion}.pdf")
    return render_template("print_nomina.html", **ctx)


# ─────────────────────────────────────────
# Helper: generación de PDF (opcional, requiere reportlab)
# ─────────────────────────────────────────
def _generar_pdf(template_name, ctx, filename):
    """Renderiza la plantilla y devuelve un PDF. Si reportlab no está disponible,
       avisa al usuario y vuelve al HTML."""
    try:
        ...
        if pisa_status.err:
            flash("No se pudo generar el PDF. Se muestra la versión HTML.", "warning")
            return render_template(template_name, **ctx)
        buf.seek(0)
        return send_file(buf, mimetype="application/pdf",
                         as_attachment=True, download_name=filename)
    except ImportError:
        flash("Para descargar PDF instale 'xhtml2pdf' (pip install xhtml2pdf). "
              "Mientras tanto, use el botón 🖨️ Imprimir.", "info")
        return render_template(template_name, **ctx)
