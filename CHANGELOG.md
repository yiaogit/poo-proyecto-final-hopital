...
### Añadido
- **Sistema de impresión profesional** con 5 tipos de documentos:
  - `print_cita.html` — Comprobante de cita (paciente)
  - `print_factura.html` — Factura con desglose de descuento por seguro (paciente)
  - `print_informe.html` — Informe clínico con IMC y clasificación OMS (paciente)
  - `print_agenda_medico.html` — Agenda profesional completa (interno)
  - `print_nomina.html` — Liquidación trimestral con desglose mensual (interno)
- **5 rutas Flask** dedicadas: `/print/cita`, `/print/factura`, `/print/informe`, `/print/agenda`, `/print/nomina`.
- **Guardado como PDF** vía diálogo nativo del navegador (sin dependencias externas, funciona en Windows/macOS/Linux).
- **Aislamiento de privacidad**: los documentos para paciente nunca exponen el DNI del médico, solo el número de colegiado.
- **Impresión cross-platform**: el sistema delega en el diálogo del navegador (Windows, macOS, Linux) sin dependencias de OS.
- **Identidad visual UEV**: tema CSS oficial (`static/css/hospital.css`) con colores corporativos `#E63946` (rojo) y `#1B2444` (azul institucional).
- **Logo UEV** en SVG y PNG, integrado en navegación y cabeceras de documentos.
- **Generador de datos sintéticos** (`seed_data.py`) que produce citas realistas garantizando que cada DNI/ID Colegiado exista y que no haya colisiones de horario.
- **Datos precargados**: 30 pacientes (antes 22), 12 médicos, **55 citas** distribuidas entre los últimos 3 meses y los próximos 6 meses.
- **Forzado de impresión a color** mediante `print-color-adjust: exact` para preservar la identidad visual UEV en papel.
- **Tipografía Inter** desde Google Fonts.
- **Botones de impresión** integrados en cuatro páginas (`pacientes.html`, `medicos.html`, `buscar.html`, `citas.html`) con dropdowns contextuales.

### Cambiado
- **`requirements.txt`** mantiene solo `Flask>=3.0` como dependencia (sin dependencias nativas de OS).
- **`base.html`** se ha rediseñado con la nueva identidad UEV.
- **Tablas y formularios** usan los nuevos botones `btn-uev-primary` y `btn-uev-secondary`.
